<template>
  <div :class="[
    ...
    <v-modal v-if="(stage === 7 && level === '1') || (stage === 6 && level === '2') || (stage === 6 && level === '3')" :isActive="isModalActive">
      <div :class="$style.modalInner">
        <achieveGold v-if="errorCount === null || errorCount === 0" :class="$style.achieve"></achieveGold>
        <achieveSilver v-else :class="$style.achieve"></achieveSilver>
        <!-- <img  :class="$style.achieve" src="../assets/achieve.svg" alt="" /> -->
        <!-- <img v-else :class="$style.achieve" src="../assets/achieve.svg" alt="" /> -->
        <p  v-if="errorCount === null && errorCount === 0" :class="[$style.modalText, $style.modalTextBottom]">
          Отличная работа! Ты получаешь золотое достижение «Специалист по вычислительной математике»!<br>
Продолжай в том же духе!
        </p>
        <p  v-else :class="[$style.modalText, $style.modalTextBottom]">
...
export default {
  ...
  setup() {
    ...
    if (level.value === '3') {
      ...
      messages.value.push(
      'Расчёты метеорологической модели производит суперкомпьютер. Благодаря своей мощности он может делать точный прогноз всего за несколько часов.',
      )
      setTimeout(() => {
        messages.value.push(
